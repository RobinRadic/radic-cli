...
if [[ "$(locale -k LC_CTYPE | sed -n 's/^charmap="\(.*\)"/\1/p')" == *"UTF-8"* ]] ; then
    ...
    LP_MARK_LOAD="<load>"        # in front of the load
    LP_MARK_TEMP="<temp>"        # in front of the temp
    LP_MARK_PROXY="<proxy>"       # indicate a proxy in use
    LP_MARK_HG="@hg"   # prompt mark in hg repositories
    LP_MARK_SVN="@svn"  # prompt mark in svn repositories
    LP_MARK_GIT="@git"         # prompt mark in git repositories
    LP_MARK_FOSSIL="@fos"         # prompt mark in fossil repositories
    LP_MARK_UNTRACKED=" -A"        # if git has untracked files
...
function doVcsColorOkai {
    ...
    LP_COLOR_UP="$GREEN$bg"    #"$GREEN"        # repository is up to date / a push have been made
    LP_COLOR_COMMITS="$yellow$bg"    #"$YELLOW"  # some commits have not been pushed
    LP_COLOR_CHANGES="$orange$bg"    #"$RED"     # there is some changes to commit
    LP_COLOR_DIFF="$red ⇋ $yellow$bg"    #"$PURPLE"     # number of lines impacted by current changes
}
